...
> **Nano Banana Pro** ≈ 基于 **Gemini 3 Pro Image** 的「工作室级」图像生成 / 编辑系统。:contentReference[oaicite:0]{index=0}  

它们的共同核心是：

- **骨干**：原生多模态、超长上下文的 **Sparse Mixture‑of‑Experts (MoE) Transformer**（Gemini 2.5 / 3 Pro）:contentReference[oaicite:1]{index=1}  
- **图像头**：挂在这颗多模态大脑上的 **image head**，统一处理 Text‑to‑Image / Image‑to‑Image / Edit / multi‑image composition。:contentReference[oaicite:2]{index=2}  
...
- **Nano Banana**  
  - 对应模型：**Gemini 2.5 Flash Image**（API: `gemini-2.5-flash-image`）:contentReference[oaicite:3]{index=3}  
  - 背后是 **Gemini 2.5 Flash**（思考模型 + 长上下文 + 多模态）。:contentReference[oaicite:4]{index=4}  

- **Nano Banana Pro**  
  - 对应模型：**Gemini 3 Pro Image**（API: `gemini-3-pro-image-preview`），Google 自己主页上直接写成 **Nano Banana Pro 🍌 – built on Gemini 3 Pro**。:contentReference[oaicite:5]{index=5}  
  - 背后是 **Gemini 3 Pro**：稀疏 MoE 多模态 Transformer，1M token context。:contentReference[oaicite:6]{index=6}  

**定位差异（非常粗略的）**：

- **Nano Banana**：  
  - 以 **Gemini 2.5 Flash** 为基座，强调 **高性能/cost‑efficient**，主打日常生图 / 编辑、高吞吐。:contentReference[oaicite:7]{index=7}  
- **Nano Banana Pro**：  
  - 以 **Gemini 3 Pro** 为基座，强调 **studio‑quality**：多角色、一致性、InfoGraphics、2K/4K、多参考图等。:contentReference[oaicite:8]{index=8}  

此外，2025‑11 之后：

- Google 在 Gemini app 里已经把 Pro 设为默认图像模型，老 Nano Banana 退到次要位置。:contentReference[oaicite:9]{index=9}  
...
- **Seedream 4.0**  
  - ByteDance 的统一 T2I + Edit + multi‑image 架构，强项是海报、商业视觉、文字排版和多参考一致性。:contentReference[oaicite:10]{index=10}  
  - 在 Gemini 3 Pro Image 的 model card 中，**对标模型仍然是 “Seedream v4 / v4 4K”**。:contentReference[oaicite:11]{index=11}  
...
- **Seedream 4.5（最新）**  
  - 官方定位为一次 **refinement‑focused upgrade**，在 4.0 基础上做全面强化：  
    ...
    - 更丰富的 **世界知识 + 空间理解**。:contentReference[oaicite:12]{index=12}  
  - 日文长文「ゼロからわかる！Seedream 4.5」里，还直接把 Seedream 4.5 和 Nano Banana Pro 对比：  
    - Seedream 4.5 更偏 **艺术感/电影感光影、高一致性、参照忠实**；  
    - Nano Banana Pro 更偏 **写实、人物、肌肤、信息图、文字密集场景**。:contentReference[oaicite:13]{index=13}  
...
- **Z‑Image**（阿里 / 通义）：  
  - 6B 参数的开源基座。  
  - 架构：**Scalable Single‑Stream Diffusion Transformer（S3‑DiT）**。:contentReference[oaicite:14]{index=14}  
  - 目标：用相对小的规模 + 单流架构 + 蒸馏，做出接近 Nano Banana Pro / Seedream 的图像质量，支持本地/小型部署。  

- **GPT‑Image‑1 / GPT‑4o Image**（OpenAI）：  
  - GPT‑4o 本身就是原生多模态 LLM，图像生成被集成到同一个模型中；:contentReference[oaicite:15]{index=15}  
  - GPT‑Image‑1 对外暴露的 text+image → image 模型，支持生成 + 编辑 + inpainting。:contentReference[oaicite:16]{index=16}  
...
来自 Gemini 2.5 Technical Report + Gemini 3 Pro Model Card：:contentReference[oaicite:18]{index=18}  
...
Google 的 **LIMoE（Language‑Image MoE）** 和 **VL‑MoE** 非常像 Gemini/Gemini‑Image 的精神前辈：:contentReference[oaicite:19]{index=19}  
...
VL‑MoE 更往前走一步：:contentReference[oaicite:20]{index=20}  
...
结合官方 model card + 产品页：:contentReference[oaicite:21]{index=21}  
...
Seedream 4.0 / Z‑Image‑Edit 论文都明确说了：它们在 **一个架构内统一 T2I + Edit + multi‑image**，只是 condition 稍微不同。:contentReference[oaicite:22]{index=22}  

---

### 3.2 Latent Diffusion 视角下的统一训练目标

虽然 Google 没公开 Nano Banana / Pro 的图像头细节，但大多数工业图像系统（Seedream、Z‑Image、Qwen‑Image 等）都使用 **latent diffusion** 或其变种。:contentReference[oaicite:23]{index=23}  
...
Z‑Image / Seedream 系列论文都明确提到类似的「编辑样本构建 + 多种 loss 混合」设计。:contentReference[oaicite:24]{index=24}  
...
另一条路线是 GPT‑4o / 部分 Janus 的做法：:contentReference[oaicite:25]{index=25}  
...
Gemini 3 Pro Image 可能也采用类似「统一序列 + image head」的设计，只是 Google 未公开图像头具体是 diffusion 还是 AR，我们最多只能把这一套当作 **参考心智模型**。:contentReference[oaicite:26]{index=26}  
...
- 文本理解靠 CLIP / 文本编码器；  
- 图像网络（U‑Net / DiT）在去噪时通过 cross‑attention“看一眼”文本 embedding。:contentReference[oaicite:27]{index=27}  
...
1. **Gemini 大脑先干这些事：**
   ...
   - 结合上下文多轮对话、参考图片、文档等；  
   - 如开启 **Thinking / Search Grounding**，还会先走一段 internal reasoning + 查资料。:contentReference[oaicite:28]{index=28}  
...
- Gemini 2.5 / 3 Pro 都是长上下文多模态思考模型（百万级别 context）。:contentReference[oaicite:29]{index=29}  
- Gemini 3 Pro Image（Nano Banana Pro）明确支持：  
  - **Thinking 模式**；  
  - **Search Grounding**（生成前可查询网络/知识）。:contentReference[oaicite:30]{index=30}  
...
- 产品侧：Nano Banana Pro 支持多参考图（Google AI Studio 页面写着“consistent heroes, up to multiple subjects and images”）。:contentReference[oaicite:31]{index=31}  
...
- 前面 diffusion + identity loss 已经说明了一种标准做法：  
  ...
  - perceptual / CLIP loss 强迫编辑区域满足文本。:contentReference[oaicite:32]{index=32}  
...
- 在长期训练中，不同 expert 会自发专精不同类型 token：  
  - 人脸 / 身体 / identity；  
  - 文本排版 / 标志 / UI；  
  - 背景 / 材质 / 光照。:contentReference[oaicite:33]{index=33}  
- Router 在看到对应 patch 时，更偏向路由到这些 expert → 形成 **角色一致性 / LOGO 稳定** 的内在能力。

> 所以 Nano Banana Pro 在官方页面才敢用“locked‑in identity across infinite variations”“consistent heroes”等说法。:contentReference[oaicite:34]{index=34}  
...
- **Z‑Image**（开源，阿里）：:contentReference[oaicite:35]{index=35}  
  - 6B 参数，单流 Diffusion Transformer（S3‑DiT）；  
  - 所有模态 token（文本、图像 latent、参考 embedding）拼成一个序列；  
  - 目标：高质量 + 高效率 + 可在 <16GB 显存上跑（Turbo 版）。  

- **Nano Banana Pro**（闭源，Google）：:contentReference[oaicite:36]{index=36}  
...
|----------------|-------------------------------------------------------------------|----------------------------------------------------------------------------|
| 主干结构       | **Single‑Stream Diffusion Transformer**：decoder‑only DiT，所有输入拼一条 token 流。:contentReference[oaicite:37]{index=37} | **Sparse MoE 多模态 Transformer**（Gemini 3 Pro），image head 挂在其上。:contentReference[oaicite:38]{index=38} |
| 参数规模       | ~6B（+ Turbo 蒸馏）:contentReference[oaicite:39]{index=39}                       | 未公开，但大幅高于 6B，有效容量数百 B 级的 MoE 背骨干（推断+model card）。:contentReference[oaicite:40]{index=40} |
| 模态支持       | text + image 为主（专注图像生成/编辑）                          | text / image / audio / video 原生多模态，图像只是众多能力之一。:contentReference[oaicite:41]{index=41} |
| 文本处理       | 外挂文本 encoder（如 Qwen‑VL 轻量版）→ 只作为条件            | 由 Gemini 3 Pro 自带的大语言模块处理，拥有强推理和世界知识。:contentReference[oaicite:42]{index=42} |
| 一致性 & 编辑 | S3‑DiT 单流结构 + 编辑专用 Z‑Image‑Edit；强调文本渲染 & 准确编辑。:contentReference[oaicite:43]{index=43} | 利用多模态 MoE + Think + Search + 更大编辑数据 + RLHF，在多角色 / 多图 / infographics benchmark 上领先。:contentReference[oaicite:44]{index=44} |
| 部署重点       | 追求「在消费硬件上也能跑」，适合企业自建 / 本地部署              | 完全云服务导向，与 Google 生态（Docs / Slides / Ads / Photos）深度集成。:contentReference[oaicite:45]{index=45} |
...
- **Nano Banana / Pro（Gemini 2.5 Flash Image / 3 Pro Image）**；:contentReference[oaicite:47]{index=47}  
- **GPT‑Image‑1 / GPT‑4o Image**。:contentReference[oaicite:48]{index=48}  
...
- 架构仍然是 **统一的 diffusion transformer + VAE**，偏 Diffusion‑first；:contentReference[oaicite:49]{index=49}  
- 但 4.5 明确强调：
  - **更强的一致性**（尤其多图编辑、参照忠实）；  
  - **更好的 dense text 排版**；  
  - **更牢的空间理解**（布局 & 比例自然）。:contentReference[oaicite:50]{index=50}  

日文那篇对比文里给的“简单分工”其实很好记：

- **Art 稍微超现实 / 氛围光影 / 高艺术感 → Seedream 4.5**  
- **更写实 / 人脸 / 肤色 / 信息图 / 文字重场景 → Nano Banana Pro**:contentReference[oaicite:51]{index=51}  
...
来自 Gemini 3 Pro Image 的 model card：:contentReference[oaicite:52]{index=52}  
...
1. **MoE 带来的超大容量**  
   - 很多 expert 可以专精某类视觉/语言技能（人脸、排版、材质、风格等）；:contentReference[oaicite:53]{index=53}  
...
3. **强化学习 + 人类偏好直接用在图像任务上**  
   - model card 明确提到：用人类偏好 / critic feedback 做 RL / 安全优化。:contentReference[oaicite:54]{index=54}  
   - 评测也用 Elo / 人类偏好 → 训练目标和评测目标直接对齐。

4. **Search Grounding + Thinkable 模式的加成**  
   - 特别是 infographics / 教育 / 事实相关图片：  
     - 先 think → 再查 → 再画，比单纯「prompt→扩散」稳很多。:contentReference[oaicite:55]{index=55}  
...
- **Gemini 2.5 Technical Report**（v5，2025‑10 更新）；:contentReference[oaicite:56]{index=56}  
- **Gemini 3 Pro Model Card**（2025‑11 v6）；:contentReference[oaicite:57]{index=57}  
- **Gemini 3 Pro Image Model Card**（2025‑11 v2）；:contentReference[oaicite:58]{index=58}  
- **Z‑Image 论文 & GitHub & 官方 blog**；:contentReference[oaicite:59]{index=59}  
- **Seedream 4.0 / 4.5 官方与第三方文档**；:contentReference[oaicite:60]{index=60}  
- **Qwen‑Image Technical Report**；  
- **GPT‑ImgEval（GPT‑4o 图像架构分析）**；  
- **OpenAI GPT‑Image‑1 官方文档 & GPT‑4o Image 官方博客** 等。:contentReference[oaicite:63]{index=63}  
...
- **LIMoE: Learning Multiple Modalities with One Sparse MoE Model**（Google Blog + Paper）:contentReference[oaicite:64]{index=64}  
- **Scaling Vision‑Language Models with Sparse MoE (VL‑MoE)**  
- **Gemini 2.5 Technical Report**；Gemini 3 Pro Model Card & Gemini 3 Pro Image Model Card。:contentReference[oaicite:65]{index=65}  
- **Z‑Image: An Efficient Image Generation Foundation Model with S3‑DiT**（论文 + blog + GitHub）。:contentReference[oaicite:66]{index=66}  
- **Seedream 4.5 官方页面 & 技术解读**:contentReference[oaicite:67]{index=67}  
- **Qwen‑Image Technical Report**；**GPT‑ImgEval**（理解 GPT‑4o 图像头）。  
